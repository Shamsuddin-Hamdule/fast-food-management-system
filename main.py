...
Emp_btn = ttk.Button(options_frame,text='Employee',command=lambda: init(Emp)).grid(row=0,column=1)
Cus_btn = ttk.Button(options_frame,text='Customer',command=lambda: init(Cus)).grid(row=0,column=2)
Menu_btn = ttk.Button(options_frame,text='Menu',command=lambda: init(Mu)).grid(row=0,column=4)
Raw_btn = ttk.Button(options_frame,text='Inventory',command=lambda: init(Ra)).grid(row=0,column=5)
Sup_btn = ttk.Button(options_frame,text='Supplier',command=lambda: init(Sup)).grid(row=0,column=3)
CusB_btn = ttk.Button(options_frame,text='Customer Bill',command=lambda: init(CusB)).grid(row=0,column=7)
SupB_btn = ttk.Button(options_frame,text='Supplier Bill',command=lambda: init(SupB)).grid(row=0,column=8)
...
#e=tk.Label(options_frame,text="Time :"+fx2,font=('Bold',25),padx=50,bg='light green').grid(row=0,column=9)
...
def Emp():   ### EMPLOYEE TABLE
    ...
    lb = tk.Label(E_head,text="Employee Page",font=('Bold',40)).grid(row=0,columnspan=4)
    E_head.pack()
...
### Menu item 1
    ...
    try:
        ...
        def on_combo_select(event):
            ...
            cur.execute('''select * FROM Menu where Menu_name=?''',(selected_item,))
            for record in cur:
                a=record[0]
                b=record[3]
            if((str(switch(selected_item)))=="Error"):
                add_Emp=ttk.Button(CusB_frame,text="               ",command=upe,state='disabled').grid(row=9,column=0)
            Mid1.set(a)
            Mmrp1.set(b)
...
### Menu item 1
    ...
    try:
        ...
        def on_combo_select(event):
            ...
            cur.execute('''select * FROM Raw_Material where Menu_name=?''',(selected_item,))
            for record in cur:
                a=record[0]
                b=record[4]
            Mid1.set(a)
